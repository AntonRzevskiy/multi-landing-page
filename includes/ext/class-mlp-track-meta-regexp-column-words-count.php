 ...
 */
class MLP_Track_Meta_Regexp_Column_Words_Count extends MLP_Track_Meta {
	...
	public function get_saving_data() {
		...
		if ( 0 < $this->track_mask->words_count ) {

			return $this->track_mask->words_count;
		}
	...
	public function treat_url( $url ) {
		...
		if ( true === isset( $url[ $origin ] ) && false === isset( $url[ $this->track_id ] ) ) {
			...
			// get unic words count
			$url[ $this->track_id ] = count( array_count_values( explode( ' ', $url[ $origin ] ) ) );
